...
The goal of regconfint is to estimate point estimate with their bootstrap confidence intervals for regularized regression coefficients. The function supports various regression families. For elastic-net, the function supports automatic alpha selection when multiple alpha values are provided, and it also allows for parallel computation to optimize performance.
